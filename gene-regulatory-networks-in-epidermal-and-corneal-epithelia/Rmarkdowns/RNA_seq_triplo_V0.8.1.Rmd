...
knitr::opts_knit$set(root.dir = "/home/jsmits/Epi_fate_specifiers")
```
...
work_dir <- '/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/'
...
#source('/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/markdown_notebooks/R_functions/plot_PCs_RNAseq.R')
source('/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/markdown_notebooks/R_functions/plot_PCs_RNAseq_V2.R')
source('/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/markdown_notebooks/R_functions/plot_dist_mat_RNAseq.R')
source('/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/markdown_notebooks/R_functions/progeny_pathway_enrichment.R')
source('/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/markdown_notebooks/R_functions/GOenrichment_clustprof_JS.R')
source('/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/markdown_notebooks/R_functions/GOenrichment_plot_counts_JS')
...
```{r set_fig_dir}
#set figure directory and creat es it if it doesnt exist
figure_dir <- paste( '/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/Epi_fate_specifiers/analysis/deseq2/output_figures_deseq2/', Sys.Date(), sep="")
# figure_dir <- '/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/Epi_fate_specifiers/analysis/deseq2/output_figures_deseq2/2021-08-17'
deseq2_dir <- paste('/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/Epi_fate_specifiers/analysis/deseq2')
...
#let rename the gene identifiers
```{r read_counts_data}
counts <- read.csv('/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/Epi_fate_specifiers/RNAseq_rerun/results/gene_counts/GRCh38.p13-counts.tsv', sep = '\t')
row.names(counts) <- counts$gene
counts$gene <- NULL
counts_pseudo <- read.csv(file='/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/Epi_fate_specifiers/analysis/scRNAseq/pseudobulk.tsv', sep = '\t')
#HOXA9 is missing from pseudo bulk
work_dir <- '/ceph/rimlsfnwi/data/moldevbio/zhou/jsmits/'


#Replace the GSM number of the downloades samples with the descriptive name from the sample file
sample_data<- read.csv2(paste(work_dir, "/Epi_fate_specifiers/RNAseq_rerun/results/log/samples.tsv",sep=''), header = TRUE, sep = '\t',  comment.char = "#")
colnames(counts) <- sample_data$descriptive_name[match(colnames(counts),sample_data$sample)]
```

Replace the ENSEMBL gene ID with the HUGO gene name, use the very convenient s2s "gene_id2name.tsv" file
```{r replace_gene_ID}
ensg_2_HUGO<- read.csv2(paste(work_dir, "/Epi_fate_specifiers/genomes/GRCh38.p13/gene_id2name.tsv",sep=''), header = FALSE, sep = '\t',  comment.char = "#")
...
write.table(counts_hgnc_f, file = paste(deseq2_dir, '/all_counts_RNAseq_bulk.tsv',sep=''), sep = '\t')

#HOXA9
counts_hgnc$hgnc_symbol <- NULL
#counts_hgnc <- counts_hgnc[row.names(counts_hgnc) %in% row.names(counts_pseudo),]
...
write.table(counts_f, file = paste(deseq2_dir, '/all_counts_RNAseq.tsv',sep=''), sep = '\t')
```

Read the info regarding all the comparisons that will be performed 
```{r}
deseq_objects_df <- read.csv2(file = paste(deseq2_dir ,  'metadata_comma.csv', sep = '/'), sep=',',stringsAsFactors=FALSE)
```

```{r}
#deseq_objects_df <- deseq_objects_df[4,]
```


```{r}
#parallel::mcaffinity(1:2)
